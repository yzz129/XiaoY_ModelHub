...
import { ArrowRight, AudioLines, Box, Check, Image, LockKeyhole, Mail, MessageSquareText, Sparkles, UserRound, Video } from 'lucide-react'
...
const withoutPunctuation = (value: string) => value.replace(/\p{P}/gu, '')
...
export function AuthPage({ onAuthenticated, onCancel }: AuthPageProps) {
  ...
  return <main className="auth-page">
    ...
    <section className="auth-card-wrap">
      <form className="auth-card" onSubmit={submit} autoComplete={savePassword ? 'on' : 'off'}>
        <header>
        ...
        {mode === 'register' && <label>
          <span>昵称</span>
          <div><UserRound /><input name="name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} autoComplete="name" placeholder="屎山留什么名" required minLength={2} /></div>
        </label>}
        <label>
          <span>邮箱</span>
          <div><Mail /><input name="username" type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete={savePassword ? 'username' : 'off'} placeholder="请输入邮箱" required /></div>
        </label>
        <label>
          <span>密码</span>
          <div><LockKeyhole /><input name="password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete={savePassword ? mode === 'login' ? 'current-password' : 'new-password' : 'off'} placeholder="至少 8 位" required minLength={8} /></div>
        </label>
        <button type="button" className="auth-save-password" aria-pressed={savePassword} onClick={() => setSavePassword((current) => !current)}>
          <span aria-hidden="true">{savePassword ? <Check /> : null}</span>
          {savePassword ? '保存密码' : '不保存密码'}
        </button>
        {error && <p className="auth-error">{withoutPunctuation(error)}</p>}
