import { Box, Clock3, Download, ExternalLink, ImageOff, RotateCcw, Trash2 } from 'lucide-react'
...
function AssetActions({ asset, onReuse, onRemove }: Pick<OutputStageProps, 'onReuse' | 'onRemove'> & { asset: GeneratedAsset }) {
...
export function OutputStage(props: OutputStageProps) {
  ...
  return <div className="output-stage-wrap">
    ...
    {!props.assets.length ? <div className="empty-stage"><div className="empty-art"><div className="art-card art-one" /><div className="art-card art-two" /><div className="art-card art-three" /><div className="art-glow" /></div><p className="stage-kicker">YOUR CANVAS AWAITS</p><h2>{props.view === 'history' ? '还没有保存的作品' : '让想象，在此成形'}</h2><p>{props.view === 'history' ? '完成的图片、视频和 3D 模型会出现在这里。' : '写下一个画面，选择视觉主题，然后开始创作。'}</p>{props.view === 'session' && <div className="empty-chips">{suggestions.map(([label, prompt]) => <button type="button" key={label} onClick={() => props.onSuggestion(prompt)}>{label}</button>)}</div>}</div>
      : props.view === 'history' ? <div className="history-grid">{props.assets.map((asset) => <article className="history-card" key={asset.id}><button type="button" className="history-media" onClick={() => props.onSelect(asset.id)} aria-label={`查看作品：${asset.prompt}`}>{asset.kind === 'image' ? <img src={asset.url} alt={asset.prompt} /> : asset.kind === 'video' ? <video src={asset.url} muted playsInline aria-label={asset.prompt} /> : <ThreeDPreview compact />}</button><div className="history-info"><div><span>{assetLabel(asset)}</span><time><Clock3 size={12} />{new Date(asset.createdAt).toLocaleDateString('zh-CN')}</time></div><p>{asset.prompt}</p><AssetActions asset={asset} onReuse={props.onReuse} onRemove={props.onRemove} /></div></article>)}</div>
        : selected ? <div className="immersive-stage"><div className="hero-media" style={{ aspectRatio: selected.kind === '3d' ? '4 / 3' : selected.settings.ratio.replace(':', '/') }}>{selected.kind === 'image' ? <img src={selected.url} alt={selected.prompt} /> : selected.kind === 'video' ? <video src={selected.url} controls loop playsInline aria-label={selected.prompt} /> : <ThreeDPreview />}</div><div className="asset-rail"><div className="asset-meta"><span>{assetLabel(selected)}</span>{selected.kind !== '3d' && <><strong>{selected.settings.ratio}</strong><strong>{selected.settings.resolution}</strong></>}{selected.kind === 'video' && <strong>{selected.settings.duration}s</strong>}{selected.kind === '3d' ? <strong>{selected.settings.threeDModel === 'hyper3d-gen2-260112' ? 'Hyper3D Gen2' : 'Seed3D 2.0'}</strong> : <strong>{styleTemplates.find((item) => item.id === selected.settings.styleId)?.name ?? '自定义主题'}</strong>}</div><AssetActions asset={selected} onReuse={props.onReuse} onRemove={props.onRemove} /></div>{props.assets.length > 1 && <div className="filmstrip">{props.assets.map((asset) => <button key={asset.id} type="button" className={selected.id === asset.id ? 'active' : ''} aria-current={selected.id === asset.id} onClick={() => props.onSelect(asset.id)}>{asset.kind === 'image' ? <img src={asset.url} alt="" /> : asset.kind === 'video' ? <video src={asset.url} muted /> : <ThreeDPreview compact />}</button>)}</div>}</div>
