  ...
  return (
    <dialog ref={ref} className="settings-dialog" onClose={onClose} aria-labelledby="settings-title">
      ...
      <div className="connection-card">
        ...
        <div><strong>{hasApiKey ? '生成服务已配置' : '尚未配置 API Key'}</strong><p>火山方舟：{hasArkApiKey ? '已连接' : '未配置'} · Agnes AI：{hasAgnesApiKey ? '已连接' : '未配置'} · SiliconFlow：{hasSiliconFlowApiKey ? '已连接' : '未配置'} · Cloudflare：{hasCloudflareApiKey ? '已连接' : '未配置'} · Pollinations：{hasPollinationsApiKey ? '已连接' : '未配置'}</p></div>
      </div>
      <dl className="model-list"><div><dt>图片模型</dt><dd>{imageModels.length} 个已接入模型</dd></div><div><dt>视频模型</dt><dd>{videoModels.length} 个已接入模型</dd></div><div><dt>3D · Seed3D</dt><dd>{arkModels.threeD}</dd></div><div><dt>3D · Hyper3D</dt><dd>{arkModels.hyperThreeD}</dd></div></dl>
