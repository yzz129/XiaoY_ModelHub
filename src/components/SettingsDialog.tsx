...
import { arkModels, hasAgnesApiKey, hasApiKey, hasArkApiKey, hasCloudflareApiKey, hasPollinationsApiKey, hasSiliconFlowApiKey } from '../lib/ark'
import { imageModels, videoModels } from '../data/models'
import { catalogModels, configuredProviders } from '../data/providerCatalog'
...
export function SettingsDialog({ open, maxVideoConcurrency, maxThreeDConcurrency, onVideoConcurrencyChange, onThreeDConcurrencyChange, onClose, onClearHistory }: SettingsDialogProps) {
  ...
  const anyConfigured = hasApiKey || configuredProviders.groq || configuredProviders.openrouter || configuredProviders.elevenlabs
  useEffect(() => {
  ...
  return (
    <dialog ref={ref} className="settings-dialog" onClose={onClose} aria-labelledby="settings-title">
      ...
      <div className="connection-card">
        <div className={anyConfigured ? 'status-dot online' : 'status-dot'}><KeyRound size={19} /></div>
        <div><strong>{anyConfigured ? '生成服务已配置' : '尚未配置 API Key'}</strong><p>火山方舟：{hasArkApiKey ? '已连接' : '未配置'} · Agnes AI：{hasAgnesApiKey ? '已连接' : '未配置'} · SiliconFlow：{hasSiliconFlowApiKey ? '已连接' : '未配置'} · Cloudflare：{hasCloudflareApiKey ? '已连接' : '未配置'} · Pollinations：{hasPollinationsApiKey ? '已连接' : '未配置'}</p><p>Groq：{configuredProviders.groq ? '已连接' : '未配置'} · OpenRouter：{configuredProviders.openrouter ? '已连接' : '未配置'} · ElevenLabs：{configuredProviders.elevenlabs ? '已连接' : '未配置'}</p></div>
      </div>
      <dl className="model-list"><div><dt>语言模型</dt><dd>{catalogModels.filter((model) => model.category === 'chat' && model.integration === 'ready').length} 个已接入模型</dd></div><div><dt>语音模型</dt><dd>{catalogModels.filter((model) => model.category === 'audio' && model.integration === 'ready').length} 个已接入模型</dd></div><div><dt>图片模型</dt><dd>{imageModels.length} 个已接入模型</dd></div><div><dt>视频模型</dt><dd>{videoModels.length} 个已接入模型</dd></div><div><dt>3D · Seed3D</dt><dd>{arkModels.threeD}</dd></div><div><dt>3D · Hyper3D</dt><dd>{arkModels.hyperThreeD}</dd></div></dl>
