...
import { arkModels, hasAgnesApiKey, hasApiKey, hasArkApiKey, hasCloudflareApiKey, hasSiliconFlowApiKey } from '../lib/ark'
  ...
  return (
    <dialog ref={ref} className="settings-dialog" onClose={onClose} aria-labelledby="settings-title">
      ...
      <div className="connection-card">
        ...
        <div><strong>{hasApiKey ? '生成服务已配置' : '尚未配置 API Key'}</strong><p>火山方舟：{hasArkApiKey ? '已连接' : '未配置'} · Agnes AI：{hasAgnesApiKey ? '已连接' : '未配置'} · SiliconFlow：{hasSiliconFlowApiKey ? '已连接' : '未配置'} · Cloudflare：{hasCloudflareApiKey ? '已连接' : '未配置'}</p></div>
      </div>
